...
def get_player_header_html(game, player_idx):
    ...
    if any(game['board_closed'].values()) and not is_eliminated and not game['is_tag_team'] and game['num_players'] > 2:
        ...
        elim_progress_html = f"<div style='font-size: 12px; color: #888; margin-top: 5px;'>KO: {' '.join(dots)}</div>"
    ...
    return f"""
    <div class="player-header" style='text-align: center; margin-bottom: 10px;'>
        <div style='{border_style} border-radius: 8px; padding: 10px; background: rgba(255, 255, 255, 0.02);'>
            <div class="player-name" style='width: 100%; height: 34px; display: flex; align-items: center; justify-content: center;'>
                <span style='
                    font-weight: bold; 
                    font-size: {font_size}; 
                    color: {text_color};
            ...
            <div class="player-ko"><span style='font-size: 20px; color: {text_color};'>KO: {game['ko_numbers'][p_key]}</span></div>
        </div>
        {elim_progress_html}
    </div>
    """
        ...
        with game_container:
            ...
            with header_cols[4]:
                ...
                if any(game['board_closed'].values()):
                    ...
                    pin_meter_html = f"""<div style='margin-top: 10px; padding-top: 8px; border-top: 1px solid #333;'>
                        <div style='font-size: 10px; color: #888; margin-bottom: 3px;'>PIN</div>
                        <div style='font-size: 18px; letter-spacing: 4px; font-weight: bold;'>{meter}</div>
                    </div>"""
                
                st.markdown(f"""
                <div style='text-align: center;'>
                    <div style='font-size: 14px; color: #888; margin-bottom: 5px;'>DARTS THIS TURN</div>
                    <div style='font-size: 20px; font-weight: bold;'>{' | '.join(dart_display)}</div>
                    {pin_meter_html}
                </div>
                """, unsafe_allow_html=True)
            ...
            if 'perfect_turn' in st.session_state and st.session_state.perfect_turn:
                ...
                if elapsed < 2:  # Show for 2 seconds
                    st.markdown(f"""
                    <div style='background: #00ff88; padding: 15px; text-align: center; border-radius: 10px; margin: 10px 0;'>
                        <h3 style='color: #000; margin: 0;'>🎯 DARTS BACK! 🎯</h3>
                    </div>
                    """, unsafe_allow_html=True)
                    time.sleep(0.1)
                    st.rerun()
            ...
            if game['game_over']:
                ...
                st.markdown(f"""
                <div style='background: #00ff88; padding: 40px; text-align: center; border-radius: 15px; margin: 40px 0;'>
                    <h1 style='color: #000; margin: 0; font-size: 48px;'>🏆 {winner_name} WINS! 🏆</h1>
                    <div style='color: #000; font-size: 24px; margin-top: 20px;'>Victory by PIN!</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            # Check if current player skipped
                ...
                st.markdown(f"""
                <div style='background: #ff4444; padding: 20px; text-align: center; border-radius: 10px; margin: 20px 0;'>
                    <h2 style='color: white; margin: 0;'>💀 {current_player} SKIPPED! 💀</h2>
                    <div style='color: white; font-size: 18px; margin-top: 10px;'>{remaining}</div>
                </div>
                """, unsafe_allow_html=True)
                ...
                for idx, num in enumerate(CRICKET_NUMBERS):
                    num_label = "BULL" if num == 'B' else ("TRIP" if num == 'T' else ("DBL" if num == 'D' else str(num)))
                    ...
                    with cols[2]:
                        if marks_c3:
                            st.markdown(f"<div style='text-align: center; font-size: 48px; height: 60px; display: flex; align-items: center; justify-content: center; font-weight: bold;'>{marks_c3}</div>", unsafe_allow_html=True)
                    
                    # C4: Marks (always used)
                    with cols[3]:
                        st.markdown(f"<div style='text-align: center; font-size: 48px; height: 60px; display: flex; align-items: center; justify-content: center; font-weight: bold;'>{marks_c4}</div>", unsafe_allow_html=True)
                    ...
                    with cols[5]:
                        st.markdown(f"<div style='text-align: center; font-size: 48px; height: 60px; display: flex; align-items: center; justify-content: center; font-weight: bold;'>{marks_c6}</div>", unsafe_allow_html=True)
                    
                    # C7: Marks (if applicable)
                    with cols[6]:
                        if marks_c7:
                            st.markdown(f"<div style='text-align: center; font-size: 48px; height: 60px; display: flex; align-items: center; justify-content: center; font-weight: bold;'>{marks_c7}</div>", unsafe_allow_html=True)
            ...
            # C6: Next Player
            with mult_cols[5]:
                if st.button("⏭️ Next Player", use_container_width=True, key="next_player"):
            ...
            # C7: Undo
            with mult_cols[6]:
                if st.button("↩️", disabled=len(st.session_state.game_history)==0, use_container_width=True, key="undo"):
